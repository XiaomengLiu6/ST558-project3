...
Next we predict heart disease using a classification tree, in which the predictor space is divided into various "regions", and the predicted value for any given observation is the most common classification among all other observations in that region. Here we train our model 5-fold cross-validation as well as different tuning parameters to find the optimal number and types of tree splits.
